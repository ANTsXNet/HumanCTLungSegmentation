                                     ...
                                     outputFile = NULL )
{
  ...
  resampledImageSize <- dim( reorientImage )
  reorientImageOnes <- antsImageClone( reorientImage ) ^ 0
  fixedParameters <- getCenterOfMass( reorientImageOnes  )
  ...
  function()
    ...
    for( i in seq_len( batchSize ) )
      ...
      referenceImageOnes <- antsImageClone( referenceImage ) ^ 0

      translationParameters <- getCenterOfMass( referenceImageOnes ) - fixedParameters
...
#      cat( "\n\n\n\n\n", batchSourceImages[[i]][1], "\n" )
#      cat( batchReferenceImages[[i]][1], "\n" )
#      cat( transforms, "\n" )
#      cat( boolInvert, "\n" )

      warpedArrayY <- as.array( warpedImageY )

#      antsImageWrite( reorientImage, "/home/nick/Data/PigLung/Scripts/Temp/reorient.nii.gz" )
#      antsImageWrite( as.antsImage( warpedArrayY ), "/home/nick/Data/PigLung/Scripts/Temp/arrayY.nii.gz" )
      ...
      for( j in seq_len( channelSize ) )
        ...
        warpedArrayX <- as.array( warpedImageX )
	       warpedArrayX[which( warpedArrayX < -1000 )] <- -1000
        warpedArrayX <- ( warpedArrayX - mean( warpedArrayX ) ) / sd( warpedArrayX )

#        antsImageWrite( as.antsImage( warpedArrayX ), "/home/nick/Data/PigLung/Scripts/Temp/arrayX.nii.gz" )
#        stop( prompt = "Press [enter] to continue\n" )
